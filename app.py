...
SET_EFFECTS = {
    "极地重金属":  {"2pc": "冰属性伤害+10%", "4pc": "触发冻结/碎冰时冰伤额外+20%，技能伤害+25%"},
    "炎狱重金属":  {"2pc": "火属性伤害+10%", "4pc": "敌人处于灼烧状态时暴击率+20%"},
    "獠牙重金属":  {"2pc": "物理伤害+10%", "4pc": "对侵蚀/混乱状态目标物理伤害额外+35%"},
    "雷暴重金属":  {"2pc": "电属性伤害+10%", "4pc": "释放EX特殊攻击或连携技后暴击伤害+20%，持续8秒"},
    "震星迪斯科":  {"2pc": "冲击力+6", "4pc": "命中弱点后异常积累+20%，持续6秒"},
    "啄木鸟电音":  {"2pc": "暴击率+8%", "4pc": "暴击后攻击力+8%，最多叠3层，持续6秒"},
    "激素朋克":    {"2pc": "攻击力+10%", "4pc": "使用EX特殊攻击后攻击力+25%，持续10秒，冷却20秒"},
    "自由蓝调":    {"2pc": "异常精通+15", "4pc": "触发异常后目标异常抗性-10%，持续8秒"},
    "摇摆爵士":    {"2pc": "全队异常精通+15", "4pc": "使用连携技或终结技后全队攻击力+15%"},
    "摇滚爵士":    {"2pc": "防御力+16%", "4pc": "受击后全队防御力+15%，持续10秒"},
    "河豚电音":    {"2pc": "能量自动回复+15%", "4pc": "释放终结技后全队攻击力+25%，持续12秒"},
    "灵魂摇滚":    {"2pc": "防御力+16%", "4pc": "使用闪避反击后防御力+30%，持续8秒"},
}
...
OCR_AUTO_PROMPT = """你是绝区零（ZZZ）驱动盘识别专家。请从这张游戏截图中提取驱动盘的属性数据，并识别驱动盘的槽位编号。

请严格按以下JSON格式输出，不要添加任何其他文字：
{
  "slot": <驱动盘槽位编号，1-6的整数；通常显示在驱动盘图标旁或界面左侧（如"1号位"）；若无法判断填0>,
...
def call_ai_vision(image_b64, mime_type, provider=None, api_key=None, api_base=None, model=None, prompt=None):
...
@app.route("/api/ocr_auto", methods=["POST"])
def api_ocr_auto():
    """OCR with auto slot detection — returns slot + disc data."""
    body = request.get_json()
    if not body or not body.get("image"):
        return jsonify({"error": "缺少图片数据"}), 400
    ...
    try:
        raw = call_ai_vision(image_b64, mime_type, provider=provider, api_key=api_key,
                             api_base=api_base, model=model, prompt=OCR_AUTO_PROMPT)
        result = parse_ai_response(raw)
    except ValueError as e:
        return jsonify({"error": str(e)}), 500
    except json.JSONDecodeError:
